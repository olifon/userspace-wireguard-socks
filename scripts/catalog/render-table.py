...
CATALOG_APPS = [
    ...
    ("test-odoo-suite", "wrapped test suite", "Odoo 18's upstream `-i base --test-enable` suite — 1329 real tests across ~12 modules. Pass = wrapped pass/fail/error counts match the unwrapped baseline exactly. Catches any wrapper regression that changes even one test outcome."),
    ("wordpress", "PHP CMS lifecycle", "Full WordPress lifecycle through wrapper: wp-cli downloads core, configs against MariaDB, installs, activates a plugin; peer's wrapped curl hits wp-json/wp/v2/types and asserts the REST API returns the canonical post + page types."),
