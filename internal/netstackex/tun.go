...
// CreateNetTUN builds a WireGuard-compatible TUN device backed by a gVisor
// channel endpoint. Packets written by wireguard-go are injected into gVisor;
// packets emitted by gVisor are read back by wireguard-go and encrypted.
func CreateNetTUN(localAddresses, dnsServers []netip.Addr, mtu int) (tun.Device, *Net, error) {
	opts := stack.Options{
		NetworkProtocols:   []stack.NetworkProtocolFactory{ipv4.NewProtocol, ipv6.NewProtocol},
		TransportProtocols: []stack.TransportProtocolFactory{tcp.NewProtocol, udp.NewProtocol, icmp.NewProtocol6, icmp.NewProtocol4},
		// Promiscuous transparent proxying creates temporary local addresses for
		// packet destinations. With HandleLocal enabled, gVisor also treats the
		// peer source as temporary-local and drops the packet as martian.
		HandleLocal: false,
	}
	dev := &netTun{
...
func (tun *netTun) Read(buf [][]byte, sizes []int, offset int) (int, error) {
	for {
		...
		if tun.packetFilter != nil && !tun.packetFilter(buf[0][offset:offset+n]) {
			continue
		}
		sizes[0] = n
		return 1, nil
...
// WriteNotify is gVisor's egress path: packets produced by the userspace
// netstack are queued for wireguard-go to encrypt and send to a peer.
...
// DialContextTCPAddrPort opens a TCP connection from inside the userspace
// netstack, so source/destination routing is handled by gVisor and WireGuard.
func (net *Net) DialContextTCPAddrPort(ctx context.Context, addr netip.AddrPort) (*gonet.TCPConn, error) {
	fa, pn := convertToFullAddr(addr)
	return gonet.DialContextTCP(ctx, net.stack, fa, pn)
}

// DialContextTCPAddrPortWithBind dials a TCP endpoint from a caller-selected
// local tunnel address. It is used for PROXY-protocol-aware forwards that need
// to preserve the original source address inside the userspace netstack.
func (net *Net) DialContextTCPAddrPortWithBind(ctx context.Context, laddr, raddr netip.AddrPort) (*gonet.TCPConn, error) {
	remote, pn := convertToFullAddr(raddr)
	local, _ := convertToFullAddr(laddr)
	return gonet.DialTCPWithBind(ctx, net.stack, local, remote, pn)
}
