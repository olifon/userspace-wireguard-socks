...
func Run(opts Options) (int, error) {
	...
	if opts.SeccompMode == SeccompNone {
		return 0, fmt.Errorf("%w: arm64 ptrace currently requires seccomp trace stops to avoid syscall entry/exit ambiguity", ErrPtraceUnavailable)
	}
...
func (t *tracer) handleInterceptedSyscall(tid int, seccompStop bool) error {
	...
	nr, err := t.getSyscallNo(tid, regs)
	if err != nil {
		if errors.Is(err, syscall.ESRCH) {
			return nil
		}
...
func (t *tracer) dispatchInterceptedSyscall(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	...
	case unix.SYS_DUP3:
		return t.handleDupN(tid, regs, true, seccompStop)
...
func (t *tracer) handleSocket(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	domain := int(int32(regs.Regs[0]))
	typ := int(int32(regs.Regs[1]))
	proto := int(int32(regs.Regs[2]))
...
func (t *tracer) handleConnect(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	fd := int(int32(regs.Regs[0]))
	state := t.trackedSnapshot(tid, fd)
	if state.Active == 0 {
		return t.resumeDefault(tid, seccompStop)
	}
	addr, err := t.readSockaddr(tid, uintptr(regs.Regs[1]), regs.Regs[2])
...
func (t *tracer) handleAccept(tid int, regs unix.PtraceRegs, accept4 bool, seccompStop bool) error {
	...
	flags := 0
	if accept4 {
		flags = int(int32(regs.Regs[3])) & (unix.SOCK_NONBLOCK | unix.SOCK_CLOEXEC)
	}
	addrPtr := regs.Regs[1]
	addrLen := regs.Regs[2]
	regs.Regs[8] = uint64(unix.SYS_SOCKET)
	regs.Regs[0] = uint64(state.Domain)
	regs.Regs[1] = uint64(unix.SOCK_STREAM | flags)
	regs.Regs[2] = 0
	regs.Regs[3] = 0
	regs.Regs[4] = 0
	regs.Regs[5] = 0
	if err := t.setSyscallNo(tid, unix.SYS_SOCKET); err != nil {
		_ = unix.Close(managerLocal)
		if errors.Is(err, syscall.ESRCH) {
			return nil
		}
		return fmt.Errorf("set accept replacement syscall on tid %d: %w", tid, err)
	}
	if err := unix.PtraceSetRegs(tid, &regs); err != nil {
		if errors.Is(err, syscall.ESRCH) {
			_ = unix.Close(managerLocal)
...
func (t *tracer) handleReadv(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	fd := int(int32(regs.Regs[0]))
	iovPtr := uintptr(regs.Regs[1])
	iovLen := uint64(regs.Regs[2])
	state := t.trackedSnapshot(tid, fd)
	...
	// readv has no flags; rely on the socket's O_NONBLOCK via SavedFL.
...
func (t *tracer) handleSendto(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	...
	if state.Active != 0 && state.Proxied == 0 && int(state.Type)&0xf == unix.SOCK_DGRAM {
		...
		} else if dest, err := t.readSockaddr(tid, uintptr(regs.Regs[4]), regs.Regs[5]); err == nil && (dest.family == unix.AF_INET || dest.family == unix.AF_INET6) && !isLoopback(dest) {
			if isICMPProtocol(int(state.Protocol)) {
	...
	buf, err := t.readTraceeBytes(tid, uintptr(regs.Regs[1]), int(regs.Regs[2]))
	if err != nil {
		return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
	}
	switch state.Kind {
	...
	case uwgshared.KindUDPConnected:
		if regs.Regs[4] != 0 && !isICMPProtocol(int(state.Protocol)) {
			return t.finishEmulated(tid, regs, -int64(syscall.EISCONN), seccompStop)
		}
	...
	case uwgshared.KindUDPListener:
		...
		dest, err := t.readSockaddr(tid, uintptr(regs.Regs[4]), regs.Regs[5])
		if err != nil {
			return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
		}
...
func (t *tracer) handleRecvfrom(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	...
	case uwgshared.KindTCPStream:
		...
		if n > 0 {
			if err := t.writeTracee(tid, uintptr(regs.Regs[1]), out[:n]); err != nil {
				return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
			}
		}
		if regs.Regs[4] != 0 && regs.Regs[5] != 0 {
			sa, saLen := sockaddrBytes(tracedSockaddr{
				family: int(state.RemoteFamily),
				ip:     uwgshared.BytesToString(state.RemoteIP[:]),
				port:   state.RemotePort,
			})
			_ = t.writeTracee(tid, uintptr(regs.Regs[4]), sa)
			_ = t.writeUint32(tid, uintptr(regs.Regs[5]), uint32(saLen))
		}
		return t.finishEmulated(tid, regs, int64(n), seccompStop)
	case uwgshared.KindUDPConnected:
		// Same MSG_DONTWAIT / O_NONBLOCK propagation as the TCP path.
		// On arm64 the recvfrom flags arg is Regs[3] (4th positional).
		recvFlags := int(int32(regs.Regs[3]))
		if state.SavedFL&unix.O_NONBLOCK != 0 {
		...
		out := packet
		if len(out) > int(regs.Regs[2]) {
			out = out[:int(regs.Regs[2])]
		}
		if err := t.writeTracee(tid, uintptr(regs.Regs[1]), out); err != nil {
			return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
		}
		if regs.Regs[4] != 0 && regs.Regs[5] != 0 {
			sa, saLen := sockaddrBytes(tracedSockaddr{
...
func (t *tracer) handleRecvmsg(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	fd := int(int32(regs.Regs[0]))
	msgPtr := uintptr(regs.Regs[1])
	// recvmsg's flags arg is the 3rd positional → arm64 Regs[2].
	// Previously dropped — see amd64 handler for the chromium /
	// libuv / Go-netpoller hang it caused.
	flags := int(int32(regs.Regs[2]))
	state := t.trackedSnapshot(tid, fd)
	if state.Active == 0 && state.Proxied == 0 {
		return t.resumeDefault(tid, seccompStop)
	}
...
func (t *tracer) handleSendmmsg(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	fd := int(int32(regs.Regs[0]))
	vecPtr := uintptr(regs.Regs[1])
	vlen := uint64(regs.Regs[2])
	state := t.trackedSnapshot(tid, fd)
	if state.Active == 0 && state.Proxied == 0 {
		return t.resumeDefault(tid, seccompStop)
	}
...
func (t *tracer) handleRecvmmsg(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	...
	vecPtr := uintptr(regs.Regs[1])
	vlen := uint64(regs.Regs[2])
	// recvmmsg's flags is the 4th positional → arm64 Regs[3].
	flags := int(int32(regs.Regs[3]))
	state := t.trackedSnapshot(tid, fd)
	if state.Active == 0 && state.Proxied == 0 {
		return t.resumeDefault(tid, seccompStop)
...
// See tracer_linux_amd64.go::emulateRecvBuffer for the rationale on
// callerFlags propagation. Same MSG_DONTWAIT / O_NONBLOCK plumbing.
...
func (t *tracer) handleGetSockName(tid int, regs unix.PtraceRegs, peer bool, seccompStop bool) error {
	...
	if regs.Regs[1] != 0 && regs.Regs[2] != 0 {
		if t.verbose {
		...
		if err := t.writeTracee(tid, uintptr(regs.Regs[1]), raw); err != nil {
			return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
		}
		if err := t.writeUint32(tid, uintptr(regs.Regs[2]), uint32(rawLen)); err != nil {
			return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
		}
	}
	return t.finishEmulated(tid, regs, 0, seccompStop)
...
func (t *tracer) handleSelect(tid int, regs unix.PtraceRegs, pselect bool, seccompStop bool) error {
	...
	readPtr := uintptr(regs.Regs[1])
	writePtr := uintptr(regs.Regs[2])
	exceptPtr := uintptr(regs.Regs[3])
	timeoutPtr := uintptr(regs.Regs[4])
...
func (t *tracer) pollTimeoutMillis(tid int, regs unix.PtraceRegs, ppoll bool) (int, error) {
	if !ppoll {
		return int(int32(regs.Regs[2])), nil
	}
	if regs.Regs[2] == 0 {
		return -1, nil
	}
	raw, err := t.readTraceeBytes(tid, uintptr(regs.Regs[2]), 16)
	if err != nil {
		return 0, err
	}
	if len(raw) < 16 {
...
func (t *tracer) handleFcntl(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	fd := int(int32(regs.Regs[0]))
	cmd := int(int32(regs.Regs[1]))
	arg := int32(regs.Regs[2])
	state := t.trackedSnapshot(tid, fd)
	if state.Proxied == 0 {
		if state.Active != 0 {
			switch cmd {
...
func (t *tracer) handleGetSockOpt(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	fd := int(int32(regs.Regs[0]))
	level := int(int32(regs.Regs[1]))
	optname := int(int32(regs.Regs[2]))
	optval := uintptr(regs.Regs[3])
	optlenp := uintptr(regs.Regs[4])
	state := t.trackedSnapshot(tid, fd)
	if t.verbose {
...
func (t *tracer) handleSetSockOpt(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	...
	if level == unix.SOL_SOCKET {
		switch optname {
		case unix.SO_REUSEADDR:
			value, err := t.readTraceeInt32(tid, uintptr(regs.Regs[3]))
			if err != nil {
				return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
			}
...
func (t *tracer) handlePendingExit(tid int, pending pendingSyscall, regs unix.PtraceRegs) error {
	...
	case pendingDupN:
		if int64(regs.Regs[0]) >= 0 && pending.state.Active != 0 {
...
func (t *tracer) finishEmulated(tid int, regs unix.PtraceRegs, result int64, seccompStop bool) error {
	...
	if err := t.setSyscallNo(tid, -1); err != nil {
		if errors.Is(err, syscall.ESRCH) {
			return nil
		}
		return fmt.Errorf("set emulated syscall number on tid %d: %w", tid, err)
	}
	if err := unix.PtraceSetRegs(tid, &regs); err != nil {
		if errors.Is(err, syscall.ESRCH) {
			return nil
	...
	exitRegs, err := t.runToSyscallExit(tid, seccompStop)
	if err != nil {
		if errors.Is(err, syscall.ESRCH) {
			return nil
		}
		return fmt.Errorf("wait emulated syscall exit on tid %d: %w", tid, err)
	}
	exitRegs.Regs[0] = uint64(result)
	if err := unix.PtraceSetRegs(tid, &exitRegs); err != nil {
		if errors.Is(err, syscall.ESRCH) {
			return nil
		}
		return fmt.Errorf("set emulated result on tid %d: %w", tid, err)
	}
	if !seccompStop {
		t.inSyscall[tid] = false
	}
...
func (t *tracer) getSyscallNo(tid int, regs unix.PtraceRegs) (int64, error) {
	nr, err := t.readSyscallNo(tid)
	if err == nil {
		return nr, nil
	}
	if t.verbose {
		fmt.Fprintf(os.Stderr, "uwgwrapper: NT_ARM_SYSTEM_CALL read failed for tid=%d: %v\n", tid, err)
	}
	return int64(regs.Regs[8]), nil
}

func (t *tracer) readSyscallNo(tid int) (int64, error) {
	var nr int32
	iov := unix.Iovec{
		Base: (*byte)(unsafe.Pointer(&nr)),
		Len:  uint64(unsafe.Sizeof(nr)),
	}
	_, _, errno := syscall.RawSyscall6(syscall.SYS_PTRACE, uintptr(unix.PTRACE_GETREGSET), uintptr(tid), uintptr(unix.NT_ARM_SYSTEM_CALL), uintptr(unsafe.Pointer(&iov)), 0, 0)
	if errno != 0 {
		return 0, errno
	}
	return int64(nr), nil
...
func (t *tracer) setSyscallNo(tid int, nr int64) error {
	v := int32(nr)
	iov := unix.Iovec{
		Base: (*byte)(unsafe.Pointer(&v)),
		Len:  uint64(unsafe.Sizeof(v)),
	}
	_, _, errno := syscall.RawSyscall6(syscall.SYS_PTRACE, uintptr(unix.PTRACE_SETREGSET), uintptr(tid), uintptr(unix.NT_ARM_SYSTEM_CALL), uintptr(unsafe.Pointer(&iov)), 0, 0)
	if errno != 0 {
		return errno
	}
	return nil
...
// See tracer_linux_amd64.go::readPacketFDNonblock for rationale.
