...
type tracer struct {
	...
	// epollProxied tracks proxied fds registered in each epoll fd.
	// Key: (process-group-pid, epoll-fd). Value: (proxied-fd → entry).
	// epoll_ctl(ADD/MOD) for a proxied fd is emulated (not forwarded to the
	// kernel) and recorded here; epoll_wait/pwait polls the localFDs instead.
	epollProxied map[procFD]map[int]epollEntry
	statsPath    string
	stats        traceStats
}

// epollEntry records a proxied fd's registration state in a kernel epoll fd.
type epollEntry struct {
	localFD int
	events  uint32
	data    uint64 // epoll_data (typically fd or ptr cast to u64)
}
...
// handleEpollCtl intercepts epoll_ctl for proxied fds.
//
// When a proxied fd is added to an epoll fd, the real kernel fd is an
// unconnected/unbound socket — the actual data arrives on localFD (a unix
// socket connected to fdproxy), not on the original fd. Registering the real
// fd in the kernel epoll would cause epoll_wait to block forever waiting for
// data that never arrives. Instead, we block the kernel epoll_ctl call and
// record the registration ourselves; handleEpollWait substitutes localFD.
func (t *tracer) handleEpollCtl(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	epfd := int(int32(regs.Rdi))
	op := int(int32(regs.Rsi))
	fd := int(int32(regs.Rdx))
	eventPtr := uintptr(regs.R10)
	...
	case unix.EPOLL_CTL_ADD, unix.EPOLL_CTL_MOD:
		...
		// epoll_event on x86_64 is 12 bytes (EPOLL_PACKED): Events(4)+Data(8).
		if eventPtr != 0 {
			raw, err := t.readTraceeBytes(tid, eventPtr, 12)
			if err != nil {
				return t.finishEmulated(tid, regs, -errnoResult(err), seccompStop)
			}
...
// interception of epoll_ctl 233). It reads /proc/<pid>/fdinfo/<epfd>, parses
// the tfd entries, and for each registered fd that is now proxied it creates
// an epollEntry and inserts it into t.epollProxied so that handleEpollWait
// can poll localFD instead of the unconnected kernel socket.
...
// handleEpollWait handles epoll_wait (232) and epoll_pwait (281).
// Both carry the same positional args: epfd, events, maxevents, timeout.
// The sigmask arg of epoll_pwait is not forwarded — we replace the entire
// wait with a poll() on localFDs + a non-blocking epoll of the dup'd epfd
// for any non-proxied fds, so there is no kernel epoll_pwait path that
// would honor a signal mask anyway. In ptrace-seccomp mode (the only mode
// where we intercept epoll) signal delivery is already linearised through
// ptrace stops, so the omission is acceptable.
func (t *tracer) handleEpollWait(tid int, regs unix.PtraceRegs, seccompStop bool) error {
	epfd := int(int32(regs.Rdi))
	eventsPtr := uintptr(regs.Rsi)
	maxEvents := int(int32(regs.Rdx))
	timeoutMs := int(int32(regs.R10))
	...
	// Build a poll array: one entry per proxied fd's localFD.
	...
	// An epoll fd is itself poll-able: POLLIN fires when events are queued.
	dupEpfd := -1
	dupIdx := -1
	...
	for i, slot := range slots {
		...
		binary.LittleEndian.PutUint64(ev[4:12], slot.entry.data)
		evBuf = append(evBuf, ev[:]...)
		nReady++
	}

	// Collect non-proxied events from the kernel epoll fd (non-blocking).
	if dupIdx >= 0 && pollFDs[dupIdx].Revents&(unix.POLLIN|unix.POLLHUP|unix.POLLERR) != 0 {
		...
		if remaining > 0 {
			...
			if err == nil {
				for _, ev := range nonProxied[:n] {
					if nReady >= maxEvents {
						break
					}
					var evBytes [evSize]byte
					binary.LittleEndian.PutUint32(evBytes[0:4], ev.Events)
					binary.LittleEndian.PutUint32(evBytes[4:8], uint32(ev.Fd))
					binary.LittleEndian.PutUint32(evBytes[8:12], uint32(ev.Pad))
...
func (t *tracer) handlePendingExit(tid int, pending pendingSyscall, regs unix.PtraceRegs) error {
	...
	case pendingClose:
		if int64(regs.Rax) == 0 {
			...
			// Clean up epoll tracking: if the closed fd was an epoll fd,
			// remove all its proxied registrations. If it was a proxied fd,
			// remove it from any epoll map it may be registered in.
