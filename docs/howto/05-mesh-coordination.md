...
By default every peer is visible to every other peer in the mesh.  You can
annotate individual clients with a **p2p mode** so the hub controls which peers
get distributed to which requester.

### Modes

| Mode | Meaning |
|------|---------|
| `active` | This peer wants direct paths. It is distributed to other `active`/`passive` peers. |
| `passive` | Available for direct connection but does not advertise itself first. Distributed to `active` peers. |
| `remote` | No direct path desired. Distributed as a relay-only peer to all requesters. |
| `disable` | Opt out entirely. Receives only `remote`-mode peers from the hub. |

Declare the mode in the client config:
...
The client posts its declared mode to the hub during every poll cycle, before
fetching the peer list.  This means the filtering takes effect on the first
response — there is no lag cycle where a `disable` client briefly sees peers it
should not.

### Example: three-tier topology

```yaml
# VPN gateway — distributes peers but does not want direct paths itself
mesh_control:
  p2p_mode: “remote”

# Road-warrior laptop — wants direct paths when possible
mesh_control:
  p2p_mode: “active”

# Embedded sensor — should be reachable but never initiates direct paths
mesh_control:
  p2p_mode: “passive”
```

The gateway is distributed to everyone as a relay target.  Road warriors
receive both the gateway and all `active`/`passive` peers, and attempt direct
paths to each.  Sensors receive the gateway and any `active` initiators, but
their `passive` mode means they wait for the other side to knock first.

## Keepalive Probes

When `mesh_control.keepalive_subnet` is set, the hub assigns a deterministic
probe IP (inside that subnet) to each active peer and sends periodic UDP probes
over the WireGuard tunnel.  This keeps the handshake alive through NAT without
relying on WireGuard's own `PersistentKeepalive` — the probe is generated by
the engine, not the kernel WG driver.

```yaml
mesh_control:
  keepalive_subnet: “250.0.0.0/8”
```

IPs are allocated deterministically from the peer's public key bytes, so the
same peer always gets the same probe IP across hub restarts.
