...
| `php` | PHP runtime | ✅ `systrap-supervised` | — | — | — | Built-in php -S dev server bound on tunnel WG addr; peer's wrapped curl fetches a JSON-encoded response. Catches the simplest possible accept(2) → reply path (single process, no fork). |
...
| `odoo` | Python ERP | ✅ `systrap-supervised` | — | — | — | Full server smoke: wrapped odoo-bin inits a fresh DB against local postgres, starts the HTTP server, wrapped curl runs the CSRF login + JSON-RPC, asserts an authenticated admin session. |
