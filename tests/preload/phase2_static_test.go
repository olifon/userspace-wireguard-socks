...
func TestPhase2StaticBinaryEchoTCP(t *testing.T) {
	...
	// Build a glibc-static C client. We deliberately avoid the
	// CGO_ENABLED=0 Go binary for this first-pass validation: Go's
	// runtime installs its own SIGSYS handler during its early init
	// (BEFORE main() runs but AFTER _start), which clobbers the
	// handler our injected blob set up. A pure-libc C binary doesn't
	// touch SIGSYS so our handler stays intact.
	//
	// A future commit will add Go-runtime support — likely via a
	// post-init re-arm hook injected before main() runs.
