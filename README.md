...
Real production apps exercised end-to-end through `uwgwrapper`, with the
wrapper-side transport the `auto` cascade selected for each. Every cell
is a real test in `scripts/catalog/apps/`; results are regenerated by
`scripts/catalog/run-suite.sh`.

For the full per-host matrix with notes, see
**[docs/catalog/production-applications.md](docs/catalog/production-applications.md)**.

Headline coverage (29 apps, hub column):

| Category | Apps |
|----------|------|
| HTTPS clients | `curl`, `wget`, `python`, `node`, `xh`, `pip`, `git`, `gh`, `cloudflared`, `java-http` |
| TCP clients / pipes | `ssh`, `rsync-ssh` (rsync over wrapped ssh, end-to-end SHA verified) |
| Servers | `nginx`, `apache2`, `php`, `caddy`, `java-minecraft`, `electron`, `postgres-server`, `mongo-server`, `mariadb-server`, `redis-server` |
| DB clients | `postgres`, `mongo` |
| UDP / DNS / NTP | `dig`, `ntp`, `iperf3-udp`, `udp-echo-bind` (same-host loopback), `mosh` (UDP roaming) |
| QUIC | `curl-http3` (libcurl with OpenSSL-QUIC + nghttp3) |
| Python ERP | `odoo` (full server smoke: init DB, start server, CSRF login + JSON-RPC) |
| Kubernetes | `kubectl` (run pod → wait → exec websocket → logs → watch → delete), `helm-odoo` (helm-deployed Odoo + authenticated browser session) |
| ML | `pytorch-mnist` (vast.ai GPU host) |
| VM under wrapper | `qemu-alpine` (qemu KVM boots Alpine to userspace inside the wrapper) |
| Wrapped test suite | `test-python-httplib` (437 CPython upstream stdlib HTTP/RPC tests) |

If your app isn't listed and you want to know whether it works under
`uwgwrapper`, the catalog harness is the answer — drop a script under
`scripts/catalog/apps/`, run it, and the result lands in the matrix.
