...
/* SYS_SECCOMP is the si_code value the kernel sets in siginfo_t when
 * SIGSYS is generated by a SECCOMP_RET_TRAP rule. Value 1 is stable
 * across all Linux architectures (asm-generic/siginfo.h). Define a
 * fallback for freestanding builds that may not expose it via
 * <signal.h>. */
#ifndef SYS_SECCOMP
...
void uwg_sigsys_handler(int sig, siginfo_t *si, void *uctx_void) {
    ...
    /* Guard: only handle SIGSYS generated by our seccomp filter.
     * A kill(pid, SIGSYS) or tgkill has si_code SI_USER/SI_TKILL (0/-6),
     * not SYS_SECCOMP (1). If we process those as syscall traps we read
     * garbage from si_syscall and dispatch a nonsensical syscall number.
     * Simulate SIG_DFL (terminate) so the behavior is well-defined. */
    if (!si || si->si_code != SYS_SECCOMP) {
        uwg_passthrough_syscall1(SYS_exit_group, 128 + SIGSYS);
