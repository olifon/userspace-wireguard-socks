...
func NewServer(config ServerConfig) (*Server, error) { //nolint:gocognit,cyclop
	...
	for _, cfg := range server.packetConnConfigs {
		am, err := server.createAllocationManager(cfg.RelayAddressGenerator, cfg.PermissionHandler, cfg.InboundPermissionHandler)
		if err != nil {
...
func (s *Server) createAllocationManager(
	...
	inboundHandler InboundPermissionHandler,
	...
	am, err := allocation.NewManager(allocation.ManagerConfig{
		...
		InboundPermissionHandler: inboundHandler,
