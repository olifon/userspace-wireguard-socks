...
// InboundPermissionHandler decides whether relay packets from a peer IP may be
// delivered to a client allocation when no explicit TURN permission exists.
// hasPermissions reports whether the allocation already has explicit
// permissions configured.
type InboundPermissionHandler func(clientAddr net.Addr, peerIP net.IP, hasPermissions bool) (ok bool)
...
type PacketConnConfig struct {
	...
	// InboundPermissionHandler is consulted when a relayed packet arrives from a
	// peer without an explicit permission on the allocation.
