...
type ManagerConfig struct {
	...
	InboundPermissionHandler func(sourceAddr net.Addr, peerIP net.IP, hasPermissions bool) bool
...
func NewManager(config ManagerConfig) (*Manager, error) {
	...
	return &Manager{
		...
		inboundPermissionHandler: config.InboundPermissionHandler,
...
// AllowInboundWithoutPermission reports whether a relayed packet from peerIP
// may be delivered to sourceAddr when the allocation has no matching explicit
// permission.
func (m *Manager) AllowInboundWithoutPermission(sourceAddr net.Addr, peerIP net.IP, hasPermissions bool) bool {
	if m.inboundPermissionHandler == nil {
		return false
	}

	return m.inboundPermissionHandler(sourceAddr, peerIP, hasPermissions)
}
