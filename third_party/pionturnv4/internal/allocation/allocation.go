...
// HasPermissions reports whether the allocation currently has any explicit
// TURN permissions.
func (a *Allocation) HasPermissions() bool {
	a.permissionsLock.RLock()
	defer a.permissionsLock.RUnlock()
...
func (a *Allocation) packetHandler(manager *Manager) {
	...
	for {
		...
		if channel := a.GetChannelByAddr(srcAddr); channel != nil { // nolint:nestif
		...
		} else if peerIP, _, addrErr := ipnet.AddrIPPort(srcAddr); addrErr == nil && manager.AllowInboundWithoutPermission(a.fiveTuple.SrcAddr, peerIP, a.HasPermissions()) {
			udpAddr, ok := srcAddr.(*net.UDPAddr)
			if !ok {
				a.log.Errorf("Failed to send DataIndication from allocation %v %v", srcAddr, err)
			...
			a.log.Debugf("Relaying message from %s to client at %s without explicit permission",
				srcAddr,
				a.fiveTuple.SrcAddr)
			if _, err = a.TurnSocket.WriteTo(msg.Raw, a.fiveTuple.SrcAddr); err != nil {
				a.log.Errorf("Failed to send DataIndication from allocation %v %v", srcAddr, err)
